 ...
 */
export function getSdkEmbedPaths(version, baseUrl) {
...
export function buildPaymentEmbedScript(botId, paymentSdkUrl) {
  return `<script>sessionStorage.setItem("bot_id", "${botId}");</script>\n<script src="${paymentSdkUrl}" defer></script>`;
}

export function buildFaqEmbedScript(botId, faqSdkUrl, { includeBotType = false } = {}) {
