      ...
      messages.map((msg) => {
        ...
        return {
          ...
          stats: {
            pass_count: 0,
            access_count: 0,
            pass_percent: msgStats ? Math.round((msgStats.pass_count / msgStats.access_count) * 100) : 0,
            msg_id: msg.id,
            ...(msgStats || {}),
              ...
              {message.belong_to === "user" && (
                <div className="sp-body-user-side csp-body-user-side slideLeft msg_with_stats">
                  <div className="msg_stats">
                    <span className="stat">
                      <span className="stat_hover_none">{(message.stats.pass_count || 0).toString().padStart(2, "0")}</span>
                      <span className="stat_addition">{(message.stats.pass_percent || 0)}%</span>
                      <span>通過</span>
                    </span>
                    ...
                    <span className="stat">
                      {(message.stats.access_count || 0).toString().padStart(2, "0")} 対応済
                    </span>
                  </div>
