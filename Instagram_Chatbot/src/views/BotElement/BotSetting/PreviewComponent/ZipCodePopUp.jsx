...
const ZipCodePopUp = ({
  ...
  const onChangeCity = (value) => {
    ...
    let city_jis_code = citiesList.find((item) => item.city_name === value)?.city_jis_code;
  ...
  const onChangeTown = (value) => {
    ...
    const zipcode = dataTowns.find((item) => item.town_name === value)?.zip_code;
  ...
  const onSelectZipcode = () => {
    const isSplitPostalCode = indexContentZipcode && message.message_content[indexContentZipcode]?.zip_code_address?.split_postal_code;
    ...
    if (state.zipcode) {
      ...
      newErrors[`message${messageIndex}_content${indexContentZipcode}_zip_code_address`] = "";
      let newZipCodeAddress = {};

      if (isSplitPostalCode) {
        newZipCodeAddress = {
          value_post_code: state.zipcode,
          value_prefecture: state.selectedPrefecture,
          value_municipality: `${state.selectedCity}${state.selectedTown}`,
        };
      } else {
        newZipCodeAddress = {
          value_post_code_left: state.zipcode.slice(0, 3),
          value_post_code_right: state.zipcode.slice(3),
          value_prefecture: state.selectedPrefecture,
          value_municipality: `${state.selectedCity}${state.selectedTown}`,
        };
      }

      onChangeValue(indexContentZipcode, "zip_code_address", newZipCodeAddress);
      // TODO: Need refactor for this part to render only 1 time instead of 2 times
      onChangeErrors(`message${messageIndex}_content${indexContentZipcode}_zip_code_address`, "");
    }
      ...
      <div className="sp-popup-zip-code-address-body">
        ...
        <div className="sp-popup-zip-code-address-body-button">
          ...
          <div
            ...
            style={state.zipcode ? {} : { opacity: "0.5" }}
