...
export default function ZipCodeAddress({ content, prefecturesList, indexMessageRender, indexMessage, indexContent, messageContent, onChangeValue, onChangeErrors, errors, disabled, onOpen }) {
  ...
  const renderTitle = () => {
    const title = zipCodeAddress.title_require && (
  ...
  const retrieveAddressFromZipCode = (zipCode) => {
    getAddressFromZipCode(zipCode).then((res) => {
      ...
      let newZipCodeAddress = {
        value_prefecture: zipCodeAddress.is_use_dropdown ? getPrefectureIdCodeFromName(prefecture_name) : prefecture_name,
      };
      ...
      <div className="ss-message__content--user-chat-container w-100-percent">
        <InputCustom
          ...
          onChange={async (value) => {
            ...
            const zipCode = value + zipCodeAddress.value_post_code_right;
        ...
        <InputCustom
          ...
          onChange={async (value) => {
            ...
            const zipCode = zipCodeAddress.value_post_code_left + value;
  ...
  const renderPostCode = () => {
    if (zipCodeAddress.post_code === undefined) return;
    const postCodeLabel = zipCodeAddress.post_code_label?.trim() !== "" ? zipCodeAddress.post_code_label : "郵便番号";
    ...
    return (
      <div className="ss-user-setting__item-bottom">
        ...
        {zipCodeAddress.split_postal_code !== true ? renderSinglePostCode() : renderSplitPostCode()}
      </div>
    );
  };

  const renderPrefecture = () => {
    if (zipCodeAddress.prefecture === undefined) return;

    const prefectureLabel = zipCodeAddress.prefecture_label?.trim() !== "" ? zipCodeAddress.prefecture_label : "都道府県";
    let prefectureInput = null;

    if (zipCodeAddress.is_use_dropdown) {
      prefectureInput = (
        <SelectCustom
  ...
  const renderMunicipality = () => {
    if (zipCodeAddress.municipality === undefined) return;

    const municipalityLabel = zipCodeAddress.municipality_label?.trim() !== "" ? zipCodeAddress.municipality_label : "市区町村";
  ...
  const renderAddress = () => {
    if (zipCodeAddress.compact_municipality_and_address || zipCodeAddress.compact_municipality_and_address_and_building_name) return;
    if (zipCodeAddress.address === undefined) return;

    const addressLabel = zipCodeAddress.address_label?.trim() !== "" ? zipCodeAddress.address_label : "番地";
  ...
  const renderBuildingName = () => {
    if (zipCodeAddress.building_name === undefined) return;
    const buildingNameLabel = zipCodeAddress.building_name_label?.trim() !== "" ? zipCodeAddress.building_name_label : "建物名";
