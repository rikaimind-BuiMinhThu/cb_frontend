...
function PushMessage() {
  ...
  useEffect(() => {
    var bot_id = Cookies.get('bot_id');
    api.get(`/api/v1/managements/push_messages?chatbot_id=${bot_id}&page=1`).then(res => {
  ...
  function selectVariableVal(value, i) {
    ...
    if (
      value == 'current_url_param' || value == 'current_url' || value == 'current_url_title'
      || value == 'user_id' || value == 'bot_id' || value == 'preview_flg'
      || value == 'user_ip_address' || value == 'user_country' || value == 'user_device'
      || value == 'user_browser' || value == 'user_agent' || value == 'cv_datetime'
      || value == 'cv_flg' || value == 'start_datetime' || value == 'user_referer_firstopen'
      || value == 'user_referer_current'
    ) {
      for (var i = 0; i < selectobject.length; i++) {
        if (selectobject.options[i].value == 'contain')
  ...
  function savePM() {
    ...
    for (let i = 0; i < formAdd.length; i++) {
      ...
      if (!formAdd[i].name.includes('variable_id') && !formAdd[i].name.includes('operator')
        && !formAdd[i].name.includes('var') && !formAdd[i].name.includes('value')) {
        ...
        if (formAdd[i].value !== 'current_url_param' || formAdd[i].value !== 'current_url' || formAdd[i].value !== 'current_url_title'
          || formAdd[i].value !== 'user_id' || formAdd[i].value !== 'bot_id' || formAdd[i].value !== 'preview_flg'
          || formAdd[i].value !== 'user_ip_address' || formAdd[i].value !== 'user_country' || formAdd[i].value !== 'user_device'
          || formAdd[i].value !== 'user_browser' || formAdd[i].value !== 'user_agent' || formAdd[i].value !== 'cv_datetime'
          || formAdd[i].value !== 'cv_flg' || formAdd[i].value !== 'start_datetime' || formAdd[i].value !== 'user_referer_firstopen'
    ...
    if (checkedStartAt == true && checkedTitle == true) {
      ...
      console.log(user)
      api.post(`/api/v1/managements/push_messages?chatbot_id=${bot_id}`, user).then(res => {
        if (res.data.code == 1) {
  ...
  function deletePM(){
    ...
    api.delete(`/api/v1/managements/push_messages/${idDelete}`).then(res =>{
      console.log(res)
      if(res.data.code ==1){
            ...
            <Card>
              ...
              <CardBody>
                <div className='push-message-option'>
                  <div id='payment_management_setting' style={{ color: '#43b8af' }}
                    className='push-message-option-item' onClick={() => pushMessageList()}>PUSH MESSAGE LIST</div>
                  <div id='payment_management_order_his' className='push-message-option-item'
                ...
                <div id='table_push_message_list' style={{ width: "100%" }}>
                    ...
                    <tbody>
                      {listPushMessage?.map((item, i) => (
                        <tr key={i}>
                          ...
                          <td style={{ width: "20%", border: '1px solid #7186a0' }}>{item.started_at.substring(0, 19).replaceAll('T', ' ')}</td>
                          <td style={{ width: "15%", border: '1px solid #7186a0' }}>{item.subscribe_status === 'subscribe' ? 'Under delivery reservation' : 'Unsubscribed'}</td>
                          <td style={{ width: "150px", border: '1px solid #7186a0' }}>
                            <div style={{ width: "100%", display: 'flex' }}>
                              <button style={{ width: "32.33%", margin: "0px 1%", borderRadius: '5px' }}
                              onClick={()=> changeStatus(item)}>{item.subscribe_status === 'subscribe' ? "To deliver" : "Unsubscribe"}</button>
                              <button style={{ width: "32.33%", margin: "0px 1%", borderRadius: '5px' }}
                              >Edit</button>
                              <button style={{ width: "32.33%", margin: "0px 1%", borderRadius: '5px' }}
                              onClick={()=> deletePMConf(item.id)}>Delete</button>
                            </div>
                          </td>
                        </tr>
                      ))}
                  ...
                  <div style={{ width: "100%", textAlign: 'center', margin: 'auto' }}>
                    <button className='push-message-btn-adddition'
                      onClick={() => addPM()}
                    >Addition</button>
                  </div>
                </div>
                <div id='table_delivery_history' style={{ width: "100%", display: 'none' }}>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    まで
                    &emsp;<button className='push-message-btn-search'>Search</button>
                  </div>
                  <Table style={{ textAlign: 'center', tableLayout: 'fixed', overflow: 'hidden' }}>
                    <thead className="text-primary">
                      <tr>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td style={{ width: "30%", border: '1px solid #7186a0' }}>Push message name</td>
                        <td style={{ width: "15%", border: '1px solid #7186a0' }}>Delivery date & time</td>
                        <td style={{ width: "20%", border: '1px solid #7186a0' }}>Number of sent SMS</td>
                        <td style={{ width: "30%", border: '1px solid #7186a0' }}>Number of failed SMS transmissions</td>
                      </tr>
                    </tbody>
                  </Table>
                </div>
        ...
        <ModalDetail open={isOpenAddPM} onClose={() => closeAddPM()}>
          <div style={{ width: "100%", height: "630px", overflowY: "auto" }}>
            <form id='form_add_PM'>
              ...
              <div className='push-message-add-form' style={{ marginTop: '-20px', marginBottom: "5px" }}>
                <span className='push-message-span-form'></span>
                <span id='titleErr' style={{ color: 'red', display: 'none' }}>Please input message name</span>
              </div>
              <div className='push-message-add-form'>
                <span className='push-message-span-form'>
                  Sending method
                </span>
                <select id='push_message_sending_method' name='sending_method' className='push-message-input-form'>
              ...
              <div className='push-message-add-form' style={{ marginTop: '-20px', marginBottom: "5px" }}>
                <span className='push-message-span-form'></span>
                <span id='startDateTimeErr' style={{ color: 'red', display: 'none' }}>Please select date and time.</span>
              </div>
              <div className='push-message-add-form'>
                <span className='push-message-span-form'>
                  Time zone exclusions for autosend pushes
                </span>
                <span><input id='has_timezone_exclusion' name='has_timezone_exclusion' onChange={(e) => selectTimezoneExclusion(e.target.checked)} type="checkbox" style={{ marginTop: "15px" }} /></span>
              </div>
              <br />
              <div id='excludedTime' style={{ width: "100%", display: 'none' }}>
                ...
                <div className='push-message-add-form'>
                  <span className='push-message-span-form'></span>
                  <span id='timeZoneErr' style={{ width: "80%" }}>

                  </span>
                </div>
              </div>
              <div style={{ width: '98%', height: '1px', margin: '10px 1%', background: "grey" }}></div>
              <span>Target person designation</span>
              <p>*By adding conditions, you can narrow down the recipients of the push message.</p>
              <p>Currently, there are 0 recipients.<span style={{ color: "blue" }}>[update]</span></p>
              <div style={{ width: '95%', margin: "10px 2.5%" }}>
                <div style={{ width: "100%", display: 'flex' }}>
                  <div style={{ width: "15%", margin: "1% 1.5%" }}></div>
                  <select id='1stVar' name='1stVar' defaultValue={'variable'} style={{ width: "15%", margin: "1% 1%" }}>
                    <option value="variable">Variable</option>
                  </select>
                  <select name="1stVar" id="1stVar"
                    defaultValue={'last_message_datetime'} style={{ width: "30%", margin: "1% 1%" }}>
                    <option value="last_message_datetime">last_message_datetime</option>
                  </select>
                  <select name="1stOperator" defaultValue={'of'} id="operator" style={{ width: "15%", margin: "1% 1%" }}>
                ...
                {customDiv.map((item, i) => (
                  <div id={`newCDiv${i}`} style={{ width: "100%", display: 'flex' }} key={i}>
                    <select name={`newAnd${i}`} id={`newAnd${i}`} defaultValue={`and`} style={{ width: "15%", margin: "1% 1%" }}>
                      <option value="and">And</option>
                    </select>
                    <select id={`var${i}`} name={`var${i}`} defaultValue={'variable'} style={{ width: "15%", margin: "1% 1%" }}>
                      <option value="variable">Variable</option>
                    </select>
                    <select name={`variable_id${i}`} id={`variable_id${i}`}
                      defaultValue={'current_url'} style={{ width: "30%", margin: "1% 1%" }}
                    ...
                    <select name={`operator${i}`} defaultValue={'is'} id={`operator${i}`} style={{ width: "15%", margin: "1% 1%" }}>
                ...
                <button style={{ float: 'right' }}
                  onClick={(e) => newTemp(e)}>Add condition</button>
              </div>
            </form>
            <div style={{ width: '95%', margin: '5% 2.5%' }}>
              <button style={{ float: 'left' }} onClick={() => setIsOpenAddPM(false)}>Cancel</button>
              <button style={{ float: 'right' }} onClick={() => savePM()}>Save</button>
            </div>
          </div>
        </ModalDetail>
