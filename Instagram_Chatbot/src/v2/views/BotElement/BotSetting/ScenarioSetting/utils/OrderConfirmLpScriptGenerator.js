...
export function buildDefaultFieldsByGroup(selectors = getEcforceSelectors(), labels = DEFAULT_LABELS) {
...
export function createOrderConfirmFieldId(fieldsByGroup = {}) {
...
export function addOrderConfirmField(fieldsByGroup, groupKey) {
...
export function removeOrderConfirmField(fieldsByGroup, groupKey, fieldId) {
  const next = ORDER_CONFIRM_GROUP_KEYS.reduce((acc, key) => {
    acc[key] = [...(fieldsByGroup[key] || [])];
    return acc;
  }, {});
...
export function reorderOrderConfirmFields(fieldsByGroup, groupKey, fromIndex, toIndex) {
  const next = ORDER_CONFIRM_GROUP_KEYS.reduce((acc, key) => {
    acc[key] = [...(fieldsByGroup[key] || [])];
    return acc;
  }, {});
...
export function updateOrderConfirmField(fieldsByGroup, groupKey, fieldId, patch) {
  const next = ORDER_CONFIRM_GROUP_KEYS.reduce((acc, key) => {
...
export function applyEcforcePresetToFields(fieldsByGroup) {
  const next = ORDER_CONFIRM_GROUP_KEYS.reduce((acc, key) => {
    acc[key] = (fieldsByGroup[key] || []).map((field) => {
...
export function syncLegacySelectorsLabelsFromFields(fieldsByGroup) {
  const selectors = {
    customer: {},
    product: {},
    summary: {},
...
export function normalizeOrderConfirmConfig(config = {}) {
...
export function buildOrderConfirmHtmlFromFields(fieldsByGroup, dataByFieldId = {}) {
  const parts = [];
...
export function buildPreviewDataByFieldId(config = {}) {
...
export function buildOrderConfirmHtml(config = {}, dataByFieldId = null) {
  const normalized = normalizeOrderConfirmConfig(config);
...
export function buildOrderConfirmPreviewHtml(config = {}) {
  return buildOrderConfirmHtml(config, buildPreviewDataByFieldId(config));
}
